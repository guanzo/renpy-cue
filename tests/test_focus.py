...
def test_setfield_on_active_input_is_managed():
    assert _focus._cue_action_manages_input(_setfield("active_input", _focus._cue))
...
def test_setfield_on_another_field_is_not_managed():
    ...
    assert not _focus._cue_action_manages_input(_setfield("some_other", _focus._cue))
...
def test_list_action_manages_if_any_element_does():
    action = [_setfield("other", _focus._cue), _setfield("active_input", _focus._cue)]
    assert _focus._cue_action_manages_input(action)


def test_list_action_not_managed_when_none_match():
    action = [_setfield("other", _focus._cue), SimpleNamespace()]
...
def test_backdrop_filtered_by_keep(monkeypatch):
    ...
    field = _focusable(32, 434, 200, 16, _setfield("active_input", _focus._cue))
...
def test_innermost_wins_when_no_keep(monkeypatch):
    ...
    inner = _focusable(32, 434, 200, 16, _setfield("active_input", _focus._cue))
