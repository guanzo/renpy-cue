...
def test_pick_deduped_fresh_pick(monkeypatch):
    monkeypatch.setattr(_trigger.helpers, "_cue_pick_file", lambda files: "a")
    assert _trigger.helpers._cue_pick_deduped(["a", "b"], []) == "a"
...
def test_pick_deduped_collision_retries(monkeypatch):
    ...
    assert _trigger.helpers._cue_pick_deduped(["a", "b"], ["a"]) == "b"
...
def test_pick_deduped_exhausted_retries_returns_none(monkeypatch):
    ...
    picks = iter(["a", "a", "a", "a"])
    monkeypatch.setattr(_trigger.helpers, "_cue_pick_file", lambda files: next(picks))
    assert _trigger.helpers._cue_pick_deduped(["a", "b"], ["a"]) is None
...
def test_pick_deduped_single_file_pool_duplicates_none(monkeypatch):
    ...
    monkeypatch.setattr(_trigger.helpers, "_cue_pick_file", lambda files: "a")
    assert _trigger.helpers._cue_pick_deduped(["a"], ["a"]) is None


def test_pick_deduped_single_file_pool_free_returns_file(monkeypatch):
    monkeypatch.setattr(_trigger.helpers, "_cue_pick_file", lambda files: "a")
    assert _trigger.helpers._cue_pick_deduped(["a"], []) == "a"
...
def test_pick_deduped_ignores_other_picks(monkeypatch):
    ...
    monkeypatch.setattr(_trigger.helpers, "_cue_pick_file", lambda files: "b")
    assert _trigger.helpers._cue_pick_deduped(["a", "b"], ["a"]) == "b"
...
def test_marker_lead_unknown_cadence_zero():
    ...
    assert _trigger.helpers._cue_marker_lead(0.0, 1.0) == 0.0
