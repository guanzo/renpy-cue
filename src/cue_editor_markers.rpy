...
init python:
    ...
    def _cue_open_repeat_dialog():
        ...
        _cue.repeat_pattern_anchor = anchor_time
        _cue.repeat_pattern_offsets = offsets
        _cue.repeat_pattern_sel_count = len(sorted_sel)
        ...
        if dur > 0 and default_interval > 0:
            ...
            max_count = 0
        _cue.repeat_count_text = str(max_count)
    ...
    def _cue_do_repeat_pattern():
        ...
        try:
            interval = float(_cue.repeat_interval_text)
            count = int(_cue.repeat_count_text)
        except (ValueError, TypeError):
            return
